...
public class AdminOrderShowController {
	...
	public String showOrderList(Model model, Pageable pageable) {
		...
		for (Order order : orderList) {
			...
			//利用ポイントを差し引いた支払金額をセット
			orderBean.setTotal(PointCalcUtil.calcPaymentTotal(total, order.getUsedPoint()));
	...
	public String showOrder(@PathVariable int id, Model model) {
		...
		// 支払金額を算出
		int paymentTotal = PointCalcUtil.calcPaymentTotal(total, order.getUsedPoint());
