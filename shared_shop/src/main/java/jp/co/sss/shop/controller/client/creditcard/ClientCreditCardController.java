...
public class ClientCreditCardController {
	...
	@Transactional
	public String deleteComplete(Integer id, HttpSession session, Model model) {
		UserBean userBean = (UserBean) session.getAttribute("user");
		if (userBean == null) return "redirect:/login";

		CreditCard card = creditCardRepository.findById(id).orElse(null);
		if (card == null || card.getUser() == null || !userBean.getId().equals(card.getUser().getId())) {
			return "redirect:/syserror";
		}

		// 既に注文で使われたカードは外部キー制約の子レコードになるため、
		// 注文履歴側の参照を外してからカードを削除する。
		List<Order> relatedOrders = orderRepository.findByCreditCard_Id(id);
